...
require([
	  ...
	  ***/
	  onCategoryItemClick = function(index){
		...
		document.getElementById('delete-category-checkbox').innerHTML = "<input type='checkbox' id='remove-category-checkbox' style='float:left;'><label>Delete this Category</label> ";
	  ...
	  ***/
	  onSelectCategory = function(dlg,categoryid){
		  ...
		  onUpdatingTask(localStorage.selectedStatus,categoryid);
	  };
	  /***
	  ...
	  ***/
	  ...
	  onUpdatingTask = function(status,categoryid){
		  if(localStorage.isEditTask=="true"){
			  ...
			  refreshTaskList(localStorage.taskListType);
	  ...
	  ***/
	  moveToEditTaskPage = function(taskid,status){
		  ...
		  registry.byId('title').set("value",task[0].title);
		  registry.byId('description').set("value",task[0].description);
		  localStorage.selectedCategoryID_ = task[0].category;
		  refreshCategoryList(task[0].category);
	  ...
	  ***/
	  onSaveCategoryClick = function(dlg){
			...
			if(localStorage.isModify=="true"){
				...
				categoryStore.add({id:(new Date()).getTime(),name:registry.byId("new-category-name").get("value")});
			}
